...
from utils.data import load_data, get_centroid, train_test_split
    ...
    for glacier_name in config['glaciers']:
        try:
            ...
            if re.search(r"Central of .* not found", str(e)):
                ...
                traceback.print_exc()
                sys.exit()
        if config['newPrecipitation']:
            precipication_path = join(config["GPCP_PATH"], f"gpcp_precip_{central}.csv")
        else:
            precipication_path = join(config["DMI_PATH"], str(central),
                                      f"monthly_total_precipitation_{central}.csv")
        if config["ocean"] == "reanalysis":
            ocean_path = join(config["REANALYSIS_PATH"], f"ocean_{central}.csv")
        elif config["ocean"] == "sst":
            ocean_path = join(config["SST_PATH"], f"ocean_{central}.csv")
        else:
            ocean_path = None
        path_dict = {
            "smb": config["SMB_PATH"],
            "humidity": join(config["IGRA_PATH"], config["centroid_map"][central], "CalHum_std.csv"),
            "pressure": join(config["IGRA_PATH"], config["centroid_map"][central], "Pressure.csv"),
            "temperature": join(config["IGRA_PATH"], config["centroid_map"][central], "Temp.csv"),
            "cloud": join(config["DMI_PATH"], str(central), f"mean_cloud_{central}.csv"),
            "wind": join(config["DMI_PATH"], str(central), f"mean_wind_{central}.csv"),
            "precipitation": precipication_path,
            "ocean": ocean_path,
        }
        try:
        ...
        for model_name in models:
            try:
                ...
                if config['ocean']:
                    name = f"{config['ocean']}.{model_name}"
                else:
